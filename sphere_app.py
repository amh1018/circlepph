"""
Sphere PPH Explorer — Streamlit app
=====================================
Run with:
    streamlit run sphere_app.py

Features:
- 3D Sphere, Edge Comparison, Height Comparison pages
- Interactive Plotly 3D sphere view and flat (θ,φ) view
- Hoverable edge midpoints (weight + endpoint info)
- Hoverable nodes ((θ,φ) and height)
- Optional arrow edges (slow for large n)
- π/2 reference lines on persistence diagrams
- st.metric boxes for edge counts on comparison pages
- show_surface toggle for the sphere mesh
- show_height_fn for height function heatmap on sphere
"""
...
from sphere_pph import (
    Config, SpherePoint,
    build_graph, analyze, height as sphere_height,
    HeightVariant, SamplingVariant, WeightMetric,
)

st.set_page_config(page_title="Sphere PPH Explorer", layout="wide")
...
SAMPLING_OPTIONS: list[str] = ["fibonacci", "grid", "jittered", "random", "clustered"]
METRIC_OPTIONS:   list[str] = ["great_circle", "chord", "param_euclidean"]

METRIC_CAPTIONS = {
    "great_circle":   "Geodesic arc length — scales with R",
    "chord":          "Ambient Euclidean chord — scales with R",
    "param_euclidean":"Flat (θ,φ) distance — **does NOT scale with R**",
}

# =============================================================================
# Global display toggles (always in sidebar)
# =============================================================================

page = st.sidebar.radio("Page", ["3D Sphere", "Edge Comparison", "Height Comparison"])
st.sidebar.markdown("---")
...
def sphere_node_xyz(points: list[SpherePoint], R: float) -> list[tuple[float,float,float]]:
    """Scale unit vectors by R to get actual 3D positions."""
    return [(p.xyz[0]*R, p.xyz[1]*R, p.xyz[2]*R) for p in points]


def weight_color(w: float, w_min: float, w_max: float) -> str:
    t = (w - w_min) / (w_max - w_min) if w_max > w_min else 0.5
    return pc.sample_colorscale('plasma', float(np.clip(t, 0, 1)))[0]


def edge_hover_text(u: int, v: int, w: float,
                    points: list[SpherePoint], heights: list[float]) -> str:
    pu, pv = points[u], points[v]
    return (
        f"<b>Edge {u} → {v}</b><br>"
        f"weight: {w:.4f}<br><br>"
        f"<b>Source node {u}</b><br>"
        f"  θ={pu.theta/np.pi:.3f}π  φ={pu.phi/np.pi:.3f}π<br>"
        f"  height={heights[u]:.4f}<br><br>"
        f"<b>Target node {v}</b><br>"
        f"  θ={pv.theta/np.pi:.3f}π  φ={pv.phi/np.pi:.3f}π<br>"
        f"  height={heights[v]:.4f}"
    )


def node_hover_text(i: int, points: list[SpherePoint], heights: list[float]) -> str:
    p = points[i]
    return (
        f"<b>Node {i}</b><br>"
        f"θ={p.theta/np.pi:.3f}π  φ={p.phi/np.pi:.3f}π<br>"
        f"height={heights[i]:.4f}"
    )


def flat_xy(points: list[SpherePoint]) -> list[tuple[float, float]]:
    """Return (θ/π, φ/π) for each point."""
    return [(p.theta / np.pi, p.phi / np.pi) for p in points]
...
def make_barcode_figure(result, height_400: bool = True) -> go.Figure:
    ...
    all_vals = ([b for b, d in finite] + [d for b, d in finite] + infinite) \
               if (finite or infinite) else [0.0, np.pi]
...
# =============================================================================
# Height function heatmap  (flat θ/φ box)
# =============================================================================

def make_height_heatmap(height_variant: str, R: float = 1.0) -> go.Figure:
    """2-D contour heatmap of the height function over (θ, φ)."""
    theta = np.linspace(0, np.pi, 180)
    phi   = np.linspace(0, 2 * np.pi, 180)
    TH, PH = np.meshgrid(theta, phi)
    Z = np.vectorize(lambda t, p: sphere_height(t, p, height_variant, R))(TH, PH)
    ...
    fig.update_layout(
        xaxis_title='θ / π  (0 = north pole, 1 = south pole)',
        yaxis_title='φ / π  (azimuth)',
        height=360,
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
...
def make_flat_edge_traces(edges_to_draw, coords, w_min, w_max,
                           ...
                           color_override=None):
    """Return a list of Scatter line traces (or add arrow annotations to fig)."""
    ...
    for u, v, w in edges_to_draw:
        b = min(int((w - w_min) / w_range * N_BUCKETS), N_BUCKETS - 1)
...
def make_flat_node_trace(points: list[SpherePoint], heights: list[float],
                          showscale: bool = True) -> go.Scatter:
...
def make_flat_midpoint_trace(edges_to_draw, coords,
                              points: list[SpherePoint],
                              heights: list[float]) -> go.Scatter:
    mx, my, htexts = [], [], []
    for u, v, w in edges_to_draw:
        x0, y0 = coords[u]; x1, y1 = coords[v]
...
# =============================================================================
# 3-D sphere trace builders
# =============================================================================

def make_sphere_surface(R: float, opacity: float = 0.10,
                         height_variant: str | None = None) -> go.Surface:
    """Semi-transparent sphere surface, optionally coloured by height."""
    ...
    if height_variant:
        ZH = np.vectorize(lambda t, p: sphere_height(t, p, height_variant, R))(V, U)
        return go.Surface(
            x=Xs, y=Ys, z=Zs, surfacecolor=ZH,
            colorscale='RdBu_r', showscale=False,
            opacity=opacity, hoverinfo='skip',
            lighting=dict(ambient=0.9, diffuse=0.4),
        )
    return go.Surface(
        x=Xs, y=Ys, z=Zs,
...
def make_3d_edge_traces(edges_to_draw, node_xyz,
                         w_min: float, w_max: float,
                         edge_opacity: float) -> list:
    if not edges_to_draw:
        return []
    N_BUCKETS = 20
    w_range = w_max - w_min if w_max > w_min else 1.0
...
def make_3d_node_trace(points: list[SpherePoint], heights: list[float],
                        node_xyz) -> go.Scatter3d:
    h_arr = np.array(heights)
...
def make_3d_midpoint_trace(edges_to_draw, node_xyz,
                            points: list[SpherePoint],
                            heights: list[float]) -> go.Scatter3d:
    mx, my, mz, htexts = [], [], [], []
    for u, v, w in edges_to_draw:
        x0,y0,z0 = node_xyz[u]; x1,y1,z1 = node_xyz[v]
...
def base_layout(height: int = 540, title: str = '') -> dict:
    return dict(
...
def add_flat_panel(fig, G, points, heights, coords,
    ...
    if show_node_markers:
        fig.add_trace(make_flat_node_trace(points, heights, showscale=showscale), row=row, col=col)
    if show_edge_markers and edges_to_draw:
        fig.add_trace(make_flat_midpoint_trace(edges_to_draw, coords, points, heights),
                      row=row, col=col)
...
if page == "3D Sphere":
    ...
    arc_max = st.sidebar.slider("Max edge length", 0.1, 20.0, 20.0, step=0.1)

    st.sidebar.header("Display")
    view_3d         = st.sidebar.checkbox("3D sphere view", value=True)
    show_surface    = st.sidebar.checkbox("Show sphere surface", value=True)
    ...
    run_pph         = st.sidebar.checkbox("Compute PPH barcode (slow for n > 50)", value=False)

    cfg = Config(n=n, height_variant=height_var,     # type: ignore[arg-type]
                 sampling=sampling,                   # type: ignore[arg-type]
                 weight_metric=metric,                # type: ignore[arg-type]
                 double_edges=double_edges, R=R)
    ...
    node_xyz = sphere_node_xyz(points, R)
    edges_to_draw = [
    ...
    if view_3d:
        data = []
        if show_surface:
            surf = make_sphere_surface(R, opacity=surface_opacity,
                                       height_variant=height_var if show_height_fn else None)
            data.append(surf)
        ...
        coords = flat_xy(points)
        fig_flat = go.Figure()
        for t in make_flat_edge_traces(edges_to_draw, coords, w_min, w_max, edge_opacity,
                                        fig=fig_flat):
            fig_flat.add_trace(t)
        if show_node_markers:
            fig_flat.add_trace(make_flat_node_trace(points, heights))
        if show_edge_markers and edges_to_draw:
            fig_flat.add_trace(make_flat_midpoint_trace(edges_to_draw, coords, points, heights))
        fig_flat.update_layout(
            **base_layout(540, f'Flat (θ,φ) — height={height_var!r}'),
            xaxis=dict(title='θ / π', range=[-0.02, 1.02],
                       showgrid=True, gridcolor='rgba(128,128,128,0.15)'),
            yaxis=dict(title='φ / π', range=[-0.05, 2.05],
                       showgrid=True, gridcolor='rgba(128,128,128,0.15)'),
        )
        st.plotly_chart(fig_flat, use_container_width=True)

    if show_height_fn and not (view_3d and show_surface):
        st.subheader(f"Height function: {height_var}")
        st.plotly_chart(make_height_heatmap(height_var, R), use_container_width=True)
    ...
    if run_pph:
        ...
        with st.spinner("Computing PPH..."):
            result = analyze(cfg)
        st.markdown(
            f"**{result.n_bars} H₁ bars** · max death = {result.max_death:.4f}"
        )
        if result.barcode:
            ...
            with st.expander("Raw barcode"):
                for b, d in result.barcode:
                    ...
                    st.text(f"  [{b:.4f},  {d_str}]")

    st.caption(f"n={n}, R={R}, height={height_var!r}, sampling={sampling!r}, "
               f"metric={metric!r}, double_edges={double_edges}")
...
# =============================================================================
    ...
    st.markdown(
        "Compare edges visible at two different arc-length thresholds. "
    ...
    st.sidebar.header("Arc-length thresholds to compare")
    ...
    edges_a = {(u, v) for u, v in G.edges() if abs(G.edges[u, v]['weight'] - arc_a) < eps
               or G.edges[u, v]['weight'] <= arc_a}
    edges_b = {(u, v) for u, v in G.edges() if abs(G.edges[u, v]['weight'] - arc_b) < eps
               or G.edges[u, v]['weight'] <= arc_b}
    only_a  = edges_a - edges_b
    only_b  = edges_b - edges_a
    common  = edges_a & edges_b

    c1, c2, c3 = st.columns(3)
    c1.metric(f"Only at ≤ {arc_a:.2f}", len(only_a), help="orange")
    c2.metric(f"Only at ≤ {arc_b:.2f}", len(only_b), help="green")
    c3.metric("Both thresholds", len(common), help="gray")
    ...
    if view_3d:
        data = []
        if show_surface:
            data.append(make_sphere_surface(R))
        for edge_set, color in [(common, 'gray'), (only_a, 'orange'), (only_b, 'limegreen')]:
            edges = [(u, v, G.edges[u, v]['weight']) for u, v in edge_set]
            if not edges:
                continue
            aw = [w for _, _, w in edges]
            ts = make_3d_edge_traces(edges, node_xyz, min(aw), max(aw), edge_opacity)
            if not use_arrows:
                for t in ts:
                    if hasattr(t, 'line'):
                        t.line.color = color
        ...
        fig.update_layout(
            ...
            title_text=(f"height={height_var!r}, n={n}  |  "
                        f"orange=only≤{arc_a:.2f}, green=only≤{arc_b:.2f}, gray=both"),
        ...
        fig = make_subplots(rows=1, cols=2,
                            subplot_titles=[f"Arc ≤ {arc_a:.2f}  (orange = unique)",
                                            f"Arc ≤ {arc_b:.2f}  (green = unique)"],
        ...
        fig.update_xaxes(title_text='θ / π', range=[-0.02, 1.02],
                         showgrid=True, gridcolor='rgba(128,128,128,0.15)')
        fig.update_yaxes(title_text='φ / π', range=[-0.05, 2.05],
                         showgrid=True, gridcolor='rgba(128,128,128,0.15)')
        fig.update_layout(**base_layout(540,
            f"height={height_var!r}, n={n}  |  gray = both thresholds"))
        st.plotly_chart(fig, use_container_width=True)
...
# =============================================================================
    ...
    st.markdown(
        "Same sample points, two different height functions. "
        "**Orange** = edges only in A, **green** = only in B, **gray** = both "
        "(orientation-aware). Hover edge midpoints for info."
    )
    ...
    height_b = st.sidebar.selectbox("Height B", HEIGHT_OPTIONS, index=3)
    ...
    cfg_a = Config(n=n, height_variant=height_a,    # type: ignore[arg-type]
                   sampling=sampling,                # type: ignore[arg-type]
                   weight_metric=metric,             # type: ignore[arg-type]
                   double_edges=double_edges, R=R)
    cfg_b = Config(n=n, height_variant=height_b,    # type: ignore[arg-type]
                   sampling=sampling,                # type: ignore[arg-type]
                   weight_metric=metric,             # type: ignore[arg-type]
                   double_edges=double_edges, R=R)
    ...
    node_xyz_a = sphere_node_xyz(points_a, R)
    node_xyz_b = sphere_node_xyz(points_b, R)

    if view_3d:
        col1, col2 = st.columns(2)
        for col_st, G, heights, pts, node_xyz, label, edge_set, ref_set, uniq_col in [
            (col1, G_a, heights_a, points_a, node_xyz_a,
             f"Height A: {height_a}", set_a, set_b, 'orange'),
            (col2, G_b, heights_b, points_b, node_xyz_b,
             f"Height B: {height_b}", set_b, set_a, 'limegreen'),
        ]:
            ...
            for subset, col_str in [(comm_here, 'gray'), (only_here, uniq_col)]:
                edges = [(u, v, G.edges[u, v]['weight']) for u, v in subset]
                if not edges:
                    continue
                aw = [w for _, _, w in edges]
                ts = make_3d_edge_traces(edges, node_xyz, min(aw), max(aw), edge_opacity)
                if not use_arrows:
                    for t in ts:
                        if hasattr(t, 'line'):
                            t.line.color = col_str
                data.extend(ts)

            if show_node_markers:
                data.append(make_3d_node_trace(pts, heights, node_xyz))
            all_here = [(u, v, G.edges[u, v]['weight']) for u, v in edge_set]
            if show_edge_markers and all_here:
                data.append(make_3d_midpoint_trace(all_here, node_xyz, pts, heights))
        ...
        fig = make_subplots(rows=1, cols=2,
                            subplot_titles=[f"Height A: {height_a}",
                                            f"Height B: {height_b}"],
                            horizontal_spacing=0.08)
        ...
        fig.update_yaxes(title_text='φ / π', range=[-0.05, 2.05],
                         showgrid=True, gridcolor='rgba(128,128,128,0.15)')
        fig.update_layout(**base_layout(540,
            f"n={n}  |  gray = both heights"))
        st.plotly_chart(fig, use_container_width=True)

    if show_height_fn:
        st.subheader("Height function heatmaps")
        ha_col, hb_col = st.columns(2)
        with ha_col:
            st.caption(f"A: {height_a}")
            st.plotly_chart(make_height_heatmap(height_a, R), use_container_width=True)
        with hb_col:
            st.caption(f"B: {height_b}")
            st.plotly_chart(make_height_heatmap(height_b, R), use_container_width=True)

    if run_pph:
        st.subheader("PPH Barcode Comparison")
        with st.spinner("Computing PPH for A..."):
            result_a = analyze(cfg_a)
        with st.spinner("Computing PPH for B..."):
            result_b = analyze(cfg_b)
        pd_col_a, pd_col_b = st.columns(2)
        with pd_col_a:
            st.caption(f"A: {height_a} — {result_a.n_bars} bars")
            st.plotly_chart(make_barcode_figure(result_a, height_400=False),
                            use_container_width=True)
        with pd_col_b:
            st.caption(f"B: {height_b} — {result_b.n_bars} bars")
            st.plotly_chart(make_barcode_figure(result_b, height_400=False),
                            use_container_width=True)

    st.caption(f"n={n}, R={R}, sampling={sampling!r}, metric={metric!r}, "
               f"double_edges={double_edges}")
